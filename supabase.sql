...
create index if not exists itinerary_activity_tasks_group_id_idx on public.itinerary_activity_tasks(group_id);
create index if not exists itinerary_activity_tasks_itinerary_item_id_idx on public.itinerary_activity_tasks(itinerary_item_id);
create index if not exists itinerary_activity_tasks_is_completed_idx on public.itinerary_activity_tasks(is_completed);
...
revoke all on public.itinerary_activity_tasks from anon;
...
begin
  ...
  if not exists (
    select 1 from pg_publication_tables
    where pubname = 'supabase_realtime' and schemaname = 'public' and tablename = 'itinerary_activity_tasks'
  ) then
