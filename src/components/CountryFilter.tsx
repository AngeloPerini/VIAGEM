...
export function CountryFilter({
  ...
  return (
    <section className="rounded-[2rem] border border-white/70 bg-white/80 p-4 shadow-xl shadow-slate-900/10 backdrop-blur-xl dark:border-slate-700 dark:bg-slate-900/85 dark:shadow-black/30 md:p-5">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="grid grid-cols-2 gap-2 rounded-2xl bg-slate-100 p-1 dark:bg-slate-800 sm:grid-cols-4">
            ...
            return (
              <button
                ...
                className={`relative h-11 rounded-xl px-3 text-sm font-black transition ${
                  active ? 'text-white' : 'text-slate-500 hover:text-slate-950 dark:text-slate-300 dark:hover:text-slate-50'
                }`}
              >
                {active ? (
                  <motion.span
