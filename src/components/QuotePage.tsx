...
function RateCard({
  ...
  return (
    <button
      type="button"
      onClick={onSelect}
      className="min-h-[13rem] rounded-[2rem] border border-white/70 bg-white/85 p-5 text-left shadow-xl shadow-slate-900/10 transition hover:-translate-y-1 hover:bg-white dark:border-slate-700 dark:bg-slate-900/85 dark:shadow-black/30 dark:hover:bg-slate-900"
    >
      ...
      <p className="mt-5 text-sm font-black uppercase tracking-[0.16em] text-slate-400 dark:text-slate-500">{code}</p>
      <h3 className="mt-1 text-xl font-black text-slate-950 dark:text-slate-50">{rate?.name ?? code}</h3>
      <p className="mt-4 text-2xl font-black text-slate-950 dark:text-slate-50">
        {rate ? `1 ${code} = R$ ${rate.rate.toFixed(code === 'JPY' ? 4 : 2).replace('.', ',')}` : `${currencySymbols[code]} --`}
      </p>
      <div className="mt-3 flex flex-wrap gap-2 text-sm font-bold text-slate-500 dark:text-slate-300">
        <span>{rate ? `${rate.variation.toFixed(2).replace('.', ',')}%` : 'Sem cotação'}</span>
        <span>•</span>
        <span>{formatUpdatedAt(rate?.updatedAt)}</span>
      </div>
      {rate?.status === 'cached' ? (
        <p className="mt-3 text-xs font-black uppercase tracking-[0.12em] text-amber-700 dark:text-amber-200">Última salva</p>
      ) : null}
    </button>
  );
}
...
export function QuotePage({
  ...
  return (
    ...
    >
      <section className="rounded-[2rem] border border-white/70 bg-white/85 p-5 shadow-xl shadow-slate-900/10 backdrop-blur-xl dark:border-slate-700 dark:bg-slate-900/85 dark:shadow-black/30 md:p-7">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-sm font-bold uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">
              Cotação ao vivo
            </p>
            <h1 className="mt-2 text-3xl font-black text-slate-950 dark:text-slate-50 md:text-4xl">
              Moedas de viagem para BRL
            </h1>
            <p className="mt-3 max-w-3xl text-sm font-bold leading-6 text-slate-500 dark:text-slate-300">
              Base em real brasileiro, com cache local para manter a página útil quando a API oscilar.
            </p>
            {warning ? (
              <p className="mt-4 inline-flex rounded-2xl bg-amber-50 px-4 py-3 text-sm font-bold text-amber-800 dark:bg-amber-400/10 dark:text-amber-200">
                {warning}
              </p>
            ) : null}
          </div>
          <button
            ...
            onClick={onRefresh}
            disabled={isLoading}
            className="inline-flex h-12 items-center justify-center gap-2 rounded-2xl bg-slate-950 px-5 font-black text-white shadow-xl shadow-slate-900/20 transition hover:bg-teal-700 disabled:cursor-not-allowed disabled:opacity-60 dark:bg-emerald-400 dark:text-emerald-950 dark:hover:bg-emerald-300"
          >
      ...
      <section className="rounded-[2rem] border border-white/70 bg-white/80 p-5 shadow-xl shadow-slate-900/10 backdrop-blur-xl dark:border-slate-700 dark:bg-slate-900/85 dark:shadow-black/30 md:p-7">
        <div className="mb-5 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          ...
          <select
            ...
            className="h-12 rounded-2xl border border-slate-200 bg-white px-4 font-black text-slate-800 outline-none focus:border-teal-400 focus:ring-4 focus:ring-teal-100 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-50 dark:focus:border-emerald-400 dark:focus:ring-emerald-400/20"
          >
            {QUOTED_TRAVEL_CURRENCIES.map((code) => (
